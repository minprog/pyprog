...
[Queue](exam_button:tutorial_classes_queue)
...
When you submit, your class will be checked **exactly** against the specification above. If there's anything not quite right with your implementation, the feedback won't provide you with any real hints this time. Instead, try adding some additional checks to `my_queue.py` yourself to see if everything works as expected. Especially make sure that `dequeue()` throws an `AssertionError` if the queue is empty.
